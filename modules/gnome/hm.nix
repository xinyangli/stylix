  ...
  config = lib.mkIf (config.stylix.enable && config.stylix.targets.gnome.enable) {
    dconf.settings = {
      ...
      "org/gnome/desktop/interface" = {
        ...
        # i.e emacs-pgtk.
        font-name = mkIf config.stylix.fonts.enable "${sansSerif.name} ${fontSize}";
        document-font-name = mkIf config.stylix.fonts.enable "${serif.name}  ${documentFontSize}";
        monospace-font-name = mkIf config.stylix.fonts.enable "${monospace.name} ${fontSize}";
      };
